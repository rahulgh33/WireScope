...
curl -sf http://localhost:8080/health || { error "Ingest not healthy"; cat /tmp/ingest.log | tail -10; exit 1; }
curl -sf http://localhost:9091/metrics > /dev/null || { error "Aggregator not healthy"; cat /tmp/aggregator.log | tail -10; exit 1; }
